...
不过事实上，就像我开头说的，我clone了两个项目，其中一个用了babel，用于转换`jsx`之类的文件的，它是基于`electron-forge`的，为了弄清楚当时的报错我还特意去看了这个库，说实话也没怎么弄明白。它的这个`boilerplate`对我最大的启发，应该是写了一个标准的`main`和`renderer`之间通信的接口，即：
